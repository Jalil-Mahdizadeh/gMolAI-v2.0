#!/usr/bin/env python3
"""Export the frozen selected gMolAI representation on the common panel."""
...
def main() -> None:
    ...
    if args.output.exists() or args.metadata.exists():
        raise FileExistsError("Refusing to overwrite gMolAI embedding output")
    rows = read_panel_tsv(args.input)
    if not rows:
        raise RuntimeError("Cannot encode an empty common panel")
    ...
    for row in rows:
        ...
        if molecule is None:
            raise RuntimeError("gMolAI panel contains an unparsable canonical SMILES")
    ...
    if not np.isfinite(matrix).all():
        raise RuntimeError("gMolAI produced non-finite values")
    norms = np.linalg.norm(matrix.astype(np.float64), axis=1)
    if np.any(norms <= 1.0e-12):
        raise RuntimeError("gMolAI produced a zero-norm vector")
