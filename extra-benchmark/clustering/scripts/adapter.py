#!/usr/bin/env python3
"""Hash-pinned wrapper around the manuscript's frozen competitor adapter."""
...
def compatible_protocol():
    ...
    protocol["comparators"] = {
        key: value
        for key, value in protocol["models"].items()
        if isinstance(value, dict) and key != "gmolai"
    }
    return protocol
...
def main() -> None:
    if benchmark_io.sha256_file(UPSTREAM) != EXPECTED_SHA256:
        raise RuntimeError("Frozen upstream competitor adapter hash changed")
    spec = importlib.util.spec_from_file_location("_frozen_clustering_adapter", UPSTREAM)
