#!/usr/bin/env python3
"""Shared, fail-closed I/O and identity helpers for the clustering benchmark."""
...
def load_protocol(path: str | Path = PROTOCOL_PATH) -> dict[str, Any]:
    ...
    if protocol.get("schema_version") != 1:
        raise RuntimeError("Unsupported protocol schema")
    if protocol.get("protocol_status") != "frozen_before_representation_execution":
        raise RuntimeError("Protocol is not frozen before representation execution")
    return protocol


def require_regular_file(path: str | Path, expected_sha256: str | None = None) -> str:
    target = Path(path)
    ...
    if expected_sha256 is not None and observed != expected_sha256:
        raise RuntimeError(
            f"SHA-256 mismatch for {target}: expected {expected_sha256}, observed {observed}"
        )
    return observed


def relative_to_repository(path: str | Path) -> str:
    return str(Path(path).resolve().relative_to(REPOSITORY_ROOT.resolve()))
...
def write_csv(
    ...
    try:
        with os.fdopen(descriptor, "w", encoding="utf-8", newline="") as handle:
            writer = csv.DictWriter(
                handle, fieldnames=tuple(columns), lineterminator="\n", extrasaction="raise"
            )
            writer.writeheader()
...
def read_panel_tsv(path: str | Path) -> list[dict[str, str]]:
    ...
    for expected, row in enumerate(rows):
        ...
        if identity in seen:
            raise RuntimeError(f"Duplicate molecular identity in {path}: {identity}")
        if hashlib.sha256(row["canonical_smiles"].encode("utf-8")).hexdigest() != identity:
            raise RuntimeError(f"Identity hash does not match canonical SMILES at row {expected}")
        seen.add(identity)
    return rows


def panel_columns(path: str | Path) -> tuple[str, ...]:
    with Path(path).open("r", encoding="utf-8", newline="") as handle:
        fields = csv.DictReader(handle, delimiter="\t").fieldnames
    if fields is None:
        raise RuntimeError(f"Panel has no header: {path}")
    return tuple(fields)


def canonicalize_external(raw_smiles: str, cfg: dict[str, Any]):
    """Remove source-explicit H atoms, then apply the repository identity policy."""
    ...
    try:
        ...
        hydrogen_normalized = Chem.MolToSmiles(
